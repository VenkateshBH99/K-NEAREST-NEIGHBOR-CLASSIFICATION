...
public class KNN1 {
 ...
 static int z=0;
 static String[] repeat=new String[1000];
 static String[] repeat1=new String[1000];
 static LinkedHashSet<String> re=new LinkedHashSet<String>();
 static LinkedHashSet<String> re1=new LinkedHashSet<String>();
static int y1=0;
static int y2=0;
static String name;
  ...
  private static String findMajority(String[] array) {
	...
	public static void main(String[] args) throws IOException {
		...
		int k=4;
		instances= new double[2000][k];
	       List<City> cityList=new ArrayList<City>();
	       List<Result> resultList=new ArrayList<Result>();
		File file=new File("Bball.txt");
		...
		IntStream.range(0, n).forEach(i -> {
            try {
                ...
                if(re.contains(st1[1])){
                	for(int y=0;y<re.size();y++){
                		if(repeat[y].equals(st1[1])){
                			team=y+1;
                			break;
                		}
                	}
                ...
                if(re1.contains(st1[2])){
                	for(int y=0;y<re1.size();y++){
                		if(repeat1[y].equals(st1[2])){
                			position=y+1;
                			break;
                		}
                	}
                ...
                instances[z][0]=(double)team;
                instances[z][1]=(double)position;
                instances[z][2]=Double.parseDouble(st1[3]);
                instances[z][3]=Double.parseDouble(st1[4]);
                //instances[z][3]=Double.parseDouble(st1[4]);
                //instances[z][4]=Double.parseDouble(st1[4]);
                cityList.add(new City(instances[z],st1[5]));
       ...
       for(int i=0;i<re.size();i++)
       	System.out.println((i+1)+":"+repeat[i]);
       System.out.print("Enter team:");
       query[0]=sc.nextDouble();

       for(int i=0;i<re1.size();i++)
       	System.out.println((i+1)+":"+repeat1[i]);
       System.out.print("Enter Position:");
       query[1]=sc.nextDouble();

       System.out.print("Enter Height(Inches):");
       query[2]=sc.nextDouble();

       System.out.print("Enter Weight(Pounds):");
       query[3]=sc.nextDouble();
       ...
       for(City city:cityList) {
    	   ...
    	   float sum=0;
    	   float sum1=0;
    	   for(int c=0;c<city.cityAttributes.length;c++){
    	   	sum+=city.cityAttributes[c];
    	   }
    	   sum1=(float)(query[0]+query[1]+query[2]+query[3]);
      ...
      System.out.println("Age of new instance is:"+majClass);
